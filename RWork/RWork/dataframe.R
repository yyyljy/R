dataframeMat1 <- matrix(1:15,ncol=3,byrow = TRUE)
dataframeMat1
mydataframe <- data.frame(dataframeMat1)
mydataframe
dataframeMat1[,1]
mydataframe[,1]
#matrix로 정의된 데이터를 dataframe으로 변경하여
#작업하는 경우가 많음
#matrix의 1열만 char타입으로 변경
#matrix는 한 타입으로만 선언해야하므로 한 열의 타입을 변경하면
#모든 데이터의 타입이 변경 된다.
...
mydataframe$X2 #$를 이용해서 열의 이름을 접근할 수 있다
mydataframe$X3 #$를 이용해서 열의 이름을 접근할 수 있다
